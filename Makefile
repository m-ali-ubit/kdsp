...
upnew:
	...
	docker exec kdsp_django ../entrypoint python manage.py makemigrations
	docker exec kdsp_django ../entrypoint python manage.py migrate
...
migrate:
	docker exec -it kdsp_django ../entrypoint python manage.py migrate

# creates new migrations if there is any model changes
migrations:
	docker exec -it kdsp_django ../entrypoint python manage.py makemigrations

# creates a django user with all rights and privileges
superuser:
	docker exec -it kdsp_django ../entrypoint python manage.py createsuperuser

# access django container shell
shell:
	docker exec -it kdsp_django ../entrypoint /bin/sh

# opens up a terminal and starts the django shell
djangoshell:
	docker exec -it kdsp_django ../entrypoint python manage.py shell_plus
